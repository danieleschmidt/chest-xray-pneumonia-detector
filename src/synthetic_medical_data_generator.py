...
def _add_ribcage(draw: ImageDraw.Draw, width: int, height: int, add_markers: bool):
    ...
    for i in range(6):  # 6 visible rib pairs
        ...
        draw.arc(
            [left_end, y_pos - 10, left_start, y_pos + 10],
            start=0,
            end=180,
            fill=rib_color,
            width=2,
        ...
        draw.arc(
            [right_start, y_pos - 10, right_end, y_pos + 10],
            start=0,
            end=180,
            fill=rib_color,
            width=2,
...
def _add_pathological_features(
    draw: ImageDraw.Draw, width: int, height: int, pathology_type: str
):
    ...
    if pathology_type in ["pneumonia", "bacterial_pneumonia", "viral_pneumonia"]:
        ...
        for _ in range(num_areas):
            ...
            for _ in range(5):
                ...
                draw.ellipse(
                    [
                        x_center + offset_x - small_size,
                        y_center + offset_y - small_size,
                        x_center + offset_x + small_size,
                        y_center + offset_y + small_size,
                    ],
...
def _add_spine(draw: ImageDraw.Draw, width: int, height: int):
    ...
    # Vertical spine line
    draw.line(
        [center_x, height // 6, center_x, height * 5 // 6],
        fill=spine_color,
...
def create_synthetic_medical_dataset(
    ...
    print("✅ Synthetic medical dataset created successfully!")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--image-size", type=int, default=224, help="Size of generated images (square)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train-split", type=float, default=0.8, help="Fraction for training set"
    )
    parser.add_argument(
        "--val-split", type=float, default=0.1, help="Fraction for validation set"
    )
    parser.add_argument(
        "--test-split", type=float, default=0.1, help="Fraction for test set"
    )
    parser.add_argument(
        "--contrast-enhancement", action="store_true", help="Apply contrast enhancement"
    )
    parser.add_argument(
        "--anatomical-markers",
        action="store_true",
...
def main():
    ...
    if args.verbose:
        ...
        print(
            f"  Pathology Distribution: {metadata['quality_metrics']['pathology_distribution']:.2%}"
        )
