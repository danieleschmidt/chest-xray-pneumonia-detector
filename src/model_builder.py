...
if __name__ == "__main__":
    ...
    transfer_model_mobilenet = create_transfer_learning_model(
        input_shape_example, num_classes=1, base_model_name="MobileNetV2"
    )
    ...
    if input_shape_example[2] != 3:
        print(
            f"Warning: VGG16 typically expects 3 channels. Current input shape: {input_shape_example}. Adjusting to 3 channels for VGG16 example."
        )
        input_shape_vgg = (input_shape_example[0], input_shape_example[1], 3)
    else:
        input_shape_vgg = input_shape_example

    transfer_model_vgg = create_transfer_learning_model(
        input_shape_vgg, num_classes=1, base_model_name="VGG16"
    )
    ...
    print("\n" + "=" * 50 + "\n")
    print(
        "Creating a transfer learning model with MobileNetV2 base for multi-class (e.g., 5 classes)..."
    )
    transfer_model_multiclass = create_transfer_learning_model(
        input_shape_example, num_classes=5, base_model_name="MobileNetV2"
    )
    ...
    try:
        ...
        create_transfer_learning_model(
            input_shape_example, num_classes=1, base_model_name="ResNet50"
        )
    except ValueError as e:
