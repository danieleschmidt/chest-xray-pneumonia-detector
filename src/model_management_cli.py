...
from .model_registry import ModelRegistry, ModelMetadata, ABTestConfig, ModelValidationError, ModelPromotionError
from .config import config


def create_registry() -> ModelRegistry:
    """Create model registry instance with configuration."""
    registry_path = config.MODEL_REGISTRY_PATH if hasattr(config, 'MODEL_REGISTRY_PATH') else './model_registry'
    mlflow_uri = config.MLFLOW_TRACKING_URI if hasattr(config, 'MLFLOW_TRACKING_URI') else None
...
def cmd_register(args) -> None:
    ...
    metadata = ModelMetadata(
        ...
        training_config=json.loads(args.training_config) if args.training_config else {},
...
def cmd_promote(args) -> None:
    ...
    try:
        ...
        if production_model:
            print(f"   Production model: {production_model.model_id} v{production_model.version}")
            print(f"   Performance: Acc={production_model.accuracy:.3f}, F1={production_model.f1_score:.3f}")
...
def cmd_list(args) -> None:
    ...
    for model_id, model_versions in models_by_id.items():
        ...
        for model in sorted(model_versions, key=lambda m: m.version, reverse=True):
            ...
            print(f"      Accuracy: {model.accuracy:.3f} | F1: {model.f1_score:.3f} | ROC AUC: {model.roc_auc:.3f}")
...
def cmd_ab_test_start(args) -> None:
    ...
    config = ABTestConfig(
        ...
        success_metrics=args.success_metrics.split(',') if args.success_metrics else ['accuracy', 'f1_score'],
    ...
    try:
        ...
        print(f"   Control: v{args.control_version} ({(1-args.traffic_split)*100:.1f}% traffic)")
        print(f"   Treatment: v{args.treatment_version} ({args.traffic_split*100:.1f}% traffic)")
...
def cmd_ab_test_list(args) -> None:
    ...
    for test in active_tests:
        ...
        print(f"   Control: v{test.control_model_version} ({(1-test.traffic_split)*100:.1f}%)")
        print(f"   Treatment: v{test.treatment_model_version} ({test.traffic_split*100:.1f}%)")
...
def main():
    ...
    register_parser.add_argument('--version', required=True, help='Model version (semantic versioning)')
    register_parser.add_argument('--model-path', required=True, help='Path to model file')
    register_parser.add_argument('--accuracy', type=float, required=True, help='Model accuracy (0-1)')
    register_parser.add_argument('--f1-score', type=float, required=True, help='F1 score (0-1)')
    register_parser.add_argument('--roc-auc', type=float, required=True, help='ROC AUC score (0-1)')
    register_parser.add_argument('--dataset-version', required=True, help='Training dataset version')
    register_parser.add_argument('--training-config', help='Training configuration as JSON string')
    ...
    ab_subparsers = ab_parser.add_subparsers(dest='ab_command', help='A/B test operations')
    ...
    ab_start_parser.add_argument('--experiment-name', required=True, help='Experiment name')
    ab_start_parser.add_argument('--control-version', required=True, help='Control model version')
    ab_start_parser.add_argument('--treatment-version', required=True, help='Treatment model version')
    ab_start_parser.add_argument('--traffic-split', type=float, required=True, help='Traffic to treatment (0-1)')
    ab_start_parser.add_argument('--duration-days', type=int, default=7, help='Test duration in days')
    ab_start_parser.add_argument('--success-metrics', default='accuracy,f1_score', help='Success metrics (comma-separated)')
    ab_start_parser.add_argument('--min-samples', type=int, default=1000, help='Minimum sample size')
    ab_start_parser.add_argument('--confidence-level', type=float, default=0.95, help='Statistical confidence level')
    ...
    # Performance command
    perf_parser = subparsers.add_parser('performance', help='Show model performance metrics')
