...
def apply_contrast(x, contrast_range_param):
    ...
    return tf.image.stateless_random_contrast(
        ...
        seed=(np.random.randint(100), np.random.randint(100)) # Provide a seed tuple for stateless_random_contrast
    )

def create_data_generators(train_dir, val_dir, target_size=(150, 150),
                           train_batch_size=32, val_batch_size=32,
                           random_flip='horizontal', rotation_range=0,
                           brightness_range=None, contrast_range=0.0, zoom_range=0.0,
                           class_mode='binary'):
    """
    ...
    if contrast_range != 0.0:
        ...
        train_datagen_args['preprocessing_function'] = lambda x: apply_contrast(x, contrast_range_param=contrast_range)
...
def create_dummy_images_for_generator(base_dir, num_images_per_class=5):
    ...
    for class_name in class_names:
        ...
        for i in range(num_images_per_class):
            try:
                img = Image.new('RGB', (200, 200), color=('red' if class_name == "class_a" else 'blue'))
                img.save(os.path.join(path, f"img_{class_name}_{i}.png"))
            except Exception as e:
                print(f"Could not create dummy image {i} for {path}: {e}. Pillow might be needed.")
                open(os.path.join(path, f"img_{class_name}_{i}.png"), 'a').close() # Fallback
...
if __name__ == '__main__':
    ...
    create_dummy_images_for_generator(dummy_train_dir, num_images_per_class=10) # e.g. 10 images per class for train
    create_dummy_images_for_generator(dummy_val_dir, num_images_per_class=5)   # e.g. 5 images per class for val
    ...
    train_gen, val_gen = create_data_generators(
        ...
        contrast_range=0.15, # Example contrast range
        zoom_range=0.1,
    ...
    if train_gen and val_gen:
        ...
        for i in range(2): # Take 2 batches
            try:
                ...
                print(f"  Images shape: {image_batch_train.shape}, type: {image_batch_train.dtype}")
                print(f"  Labels shape: {labels_batch_train.shape}, type: {labels_batch_train.dtype}, values: {labels_batch_train}")
                if image_batch_train.shape[0] > 0 : # if batch is not empty
                    print(f"  Min/Max pixel values in images: {np.min(image_batch_train[0])}, {np.max(image_batch_train[0])}")

            except StopIteration:
                print("StopIteration: Not enough data for 2 batches in training generator.")
                break
            except Exception as e:
        ...
        for i in range(2): # Take 2 batches
            try:
                ...
                print(f"  Images shape: {image_batch_val.shape}, type: {image_batch_val.dtype}")
                print(f"  Labels shape: {labels_batch_val.shape}, type: {labels_batch_val.dtype}, values: {labels_batch_val}")
                if image_batch_val.shape[0] > 0 : # if batch is not empty
                    print(f"  Min/Max pixel values in images: {np.min(image_batch_val[0])}, {np.max(image_batch_val[0])}")

            except StopIteration:
                print("StopIteration: Not enough data for 2 batches in validation generator.")
                break
            except Exception as e:
        ...
        if hasattr(train_gen.image_data_generator, 'preprocessing_function') and \
           train_gen.image_data_generator.preprocessing_function is not None:
            print("\nPreprocessing function is set on the training ImageDataGenerator (expected due to contrast_range).")
        else:
            if contrast_range == 0.0: # If contrast_range was 0, then no preprocessing function is expected
    ...
    cleanup_dummy_data_for_generator("data_generator_dummy") # Clean up the parent directory
