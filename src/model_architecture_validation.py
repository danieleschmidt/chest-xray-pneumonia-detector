...
class ModelArchitectureValidator:
    ...
    def generate_report(self) -> str:
        ...
        for result in self.validation_results:
            ...
            report.append(f"  Parameters: {result.total_params:,} total, "
...
def validate_model_architecture(models_config: List[Dict[str, Any]]) -> List[ValidationResult]:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="JSON file with model configurations to validate"
    )
    parser.add_argument(
        "--output_json",
        type=str,
...
def main():
    ...
    if not TF_AVAILABLE:
        print("Error: TensorFlow is not available. Please install TensorFlow to use model validation.")
        sys.exit(1)
    ...
    if passed_count == total_count:
        ...
        print(f"❌ {failed_count} of {total_count} model architectures failed validation.")
